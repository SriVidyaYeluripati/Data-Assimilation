...
\newpage
\section*{Abstract}
Data assimilation (DA) is central to state estimation in nonlinear dynamical systems, such as those in atmospheric and ocean sciences, where chaotic sensitivity to initial conditions limits forecasting accuracy. Classical methods, such as three-dimensional variational assimilation (3D-Var) and Ensemble Kalman Filters (EnKF), depend on linear-Gaussian assumptions and costly iterative optimization---3D-Var relies on iterative optimization of a cost function, while EnKF employs Gaussian approximations for the ensemble distribution. The \textbf{AI-Based Data Assimilation (AI-DA)} framework, following the AI-Var approach introduced by Fablet et al.\ \cite{ai_da_fablet}, learns the analysis functional $\Phi$ (a mapping from observations and background to the maximum a-posteriori state estimate) by \emph{minimizing a differentiable 3D-Var objective} directly (self-supervised), using only observations, a background state, and the covariance matrices $B$ (background error covariance) and $R$ (observation error covariance) at training time. Crucially, no re-analysis or ground truth labels are used during training; the true state is used only for offline evaluation.
\medskip
\noindent
...
\medskip
\noindent
Two key findings emerge: (1) recurrent models (GRU/LSTM) outperform MLPs, and (2) a \textbf{stochastic resampling regime} for the employed background mean improves robustness. Performance is evaluated via test RMSE computed against the (synthetic) true state, which is used only for offline evaluation---no re-analysis labels are employed during training. The resampling strategy prevents failures such as \textbf{Attractor Escape} observed under fixed priors. These results suggest that the AI-Var scheme can be improved with network architectures catering to the nature of the DA problem; it can successfully emulate data assimilation for low-dimensional chaotic systems, provided temporal modeling and stochastic regularization are incorporated.
\newpage
\section{Introduction}
\subsection{Motivation}
\noindent
State estimation in chaotic dynamical systems---crucial for accurate forecasting---is a fundamental challenge in many disciplines in science and engineering, particularly in high-stakes fields such as numerical weather prediction (NWP) and oceanography. Chaotic systems, epitomized by the \textbf{Lorenz-63} atmospheric convection model, exhibit sensitive dependence on initial conditions, where even infinitesimal errors in the initial state or forecast grow exponentially over time, making long-term prediction practically impossible \cite{lorenz_1963}. To mitigate this divergence, \textbf{Data Assimilation (DA)} techniques optimally combine noisy observations with the model's short-term forecast (the \textit{background} state) to produce a refined \textit{analysis} state, thereby constraining the trajectory to the true dynamical manifold, reducing forecast uncertainty.
\begin{figure}[H]
\centering
...
\end{figure}
\noindent
Traditional DA methods rely on knowledge of the physical model and generally assume linearity and Gaussian error distributions. Variational approaches, such as 3D-Var, minimize a cost function that balances background information and observations but require iterative optimization and depend on accurate specification of the covariance matrices $B$ (background error covariance) and $R$ (observation error covariance). Machine learning-based data assimilation, such as AI-Var as introduced in \cite{ai_da_fablet}, replaces explicit variational optimization by learning a mapping $\Phi$ (the analysis functional) that minimizes a differentiable 3D-Var objective directly (self-supervised), using only observations, a background state, and their covariances---no ground truth or re-analysis data are required for training.
\noindent
...
The central aim of this project is to develop and rigorously evaluate a machine learning-based data assimilation framework for the AI-Var approach, utilizing the Lorenz-63 system as a demanding testbed via a simulation study. We specifically seek to \textbf{train neural network models to learn the analysis functional ($\Phi$)}---the maximum a-posteriori (MAP) state estimate given background information and observations---and evaluate their performance across systematic variations in the experimental setup.
...
\item \textbf{Evaluation Across Multiple Observability Conditions:} The learned analysis is tested under two distinct partial observation operators and one non-linear observation operator: linear single-variable (\textit{x} mode, $h(x) = x_1$), multivariate (\textit{xy} mode, $h(x) = (x_1, x_2)$), and the challenging nonlinear observation ($x^2$ mode, $h(x) = x_1^2$) to assess how the networks handle partial and nonlinear data.
...
The \textbf{AI-Based Data Assimilation (AI-DA)} framework, an adaptation of the AI-Var framework introduced in \cite{ai_da_fablet}, reframes the complex and computationally intensive DA analysis update as a learning problem in functional approximation. Instead of solving the variational optimization problem iteratively during inference, AI-DA proposes learning a parametric surrogate for the analysis functional ($\Phi$)---the maximum a-posteriori (MAP) state estimate---via a neural network.
\begin{figure}[H]
\centering
\includegraphics[width=0.8\linewidth]{classical DA.png}
\caption{3D-Var Functional/AI-DA Concept. The analysis functional $\Phi$ maps inputs (observations $y$, background $\bar{x}$, and covariances $B$, $R$) to the analysis state $\hat{x}^a$. Figure adapted from \cite{ai_da_fablet}.}
\label{fig:aida_concept}
\end{figure}
\medskip
\noindent
...
\]
Training is \emph{self-supervised}: we backpropagate through $J(\hat{x}^a)$; no analysis labels are required. While training with re-analysis data is a frequent option in other approaches, we instead investigate self-supervised training.
...
\medskip
\noindent
The problem formulation involves estimating the full 3D state vector $\mathbf{x} = (x, y, z)$ from \textbf{partial and noisy observations}. The system is evaluated under two partial linear observation operators and one non-linear observation operator to probe information completeness and nonlinearity:
\begin{itemize}
\item \textbf{Linear Single-Variable Mode (\textit{x}):} Only the first component is observed: $h(\mathbf{x})=x_1$.
\item \textbf{Multivariate Mode (\textit{xy}):} Two linear components are observed: $h(\mathbf{x})=(x_1, x_2)$.
\item \textbf{Nonlinear Mode (\textit{x²}):} A nonlinear transform of the state is observed: $h(\mathbf{x})=x_1^2$, introducing complexity and ill-posedness.
\end{itemize}
...
\subsection*{Notation}
Throughout this paper, we denote the learned analysis functional by $\Phi(\cdot)$; in code, $f_\theta$ denotes the parametric neural network representation of $\Phi$. The covariance matrices $B$ (background error covariance) and $R$ (observation error covariance) are introduced formally in Section~\ref{sec:Methods} and used consistently throughout. Training is self-supervised on the differentiable 3D-Var objective; no re-analysis or analysis labels are used. The synthetic true state $x_{\text{true}}$ is used only for offline evaluation (test RMSE).

\medskip
\noindent
The methodology deployed in this project is characterized by a high degree of control and systematic comparison, designed to isolate the performance contributions of architectural choice and background conditioning strategy within the sequential adaptation of the AI-Var framework. All experiments were conducted within a unified pipeline to ensure complete traceability and comparability across model variants, observation modes, and noise regimes.
...
\noindent\textit{Note:} All figures and tables use the named metric in their captions; when a secondary metric is shown this is explicitly stated in the caption. We report mean~$\pm$~std in the main text and provide median and IQR for quantities affected by outliers (e.g., when attractor escape occurs). Also note: a high improvement percentage is better---e.g., 90\% improvement refers to $\mathrm{RMSE}_a$ being 1/10 of $\mathrm{RMSE}_b$.
...
\end{figure}
\begin{itemize}
\item \textbf{Linear Single-Variable Mode ($x$):} Observing only the first component, $h(\mathbf{x})=x_1$.
\item \textbf{Multivariate Mode ($xy$):} Observing two linear components, $h(\mathbf{x})=(x_1, x_2)$, providing increased informational coupling. The $xy$ mode provides the most information.
\item \textbf{Nonlinear Mode ($x^2$):} Observing a nonlinear transform of the state, $h(\mathbf{x})=x_1^2$. This mode introduces significant ill-posedness, as the operator removes sign information and amplifies noise. The $x^2$ mode provides the least amount of information, followed by $x$, and $xy$ provides the most.
\end{itemize}
In all modes, observations are corrupted by additive Gaussian noise ($\epsilon \sim \mathcal{N}(0, \sigma^2 I)$) to create the artificial observations from the ground-truth trajectory. To thoroughly stress-test the method and determine "when a method breaks" \cite{lorenz_1963}, four distinct noise levels were evaluated: $\sigma \in \{0.05, 0.10, 0.50, 1.00\}$ \cite{lorenz_1963}.
...
$m$ ensemble members (with replacement) and computed the batch background mean $\bar{x}_t$ (varying with each batch) and empirical covariance $B$ from those $m$ members, where the background mean is computed from a resampled ensemble.
...
mean $\bar{x}_B$ (the static average over all available training runs) and the corresponding climatological covariance $B_{\text{clim}}$ as a static prior
...
This model serves as the \textbf{necessary baseline comparison} and diagnostic control, to introduce the problem and quantify the benefit of prior information \cite{ai_da_fablet}. The Baseline MLP is a minimal, memory-less feedforward network that \textbf{excludes all prior background information} ($\bar{x}$ or $B$). It learns a direct mapping from the most recent observation ($y_t$) to predict the analysis state $\hat{x}^a = f_\theta(y_t)$ \cite{ai_da_fablet}. The architecture comprises two linear layers with a hidden dimension of 32 and uses the \textbf{Tanh activation function}. Performance results confirm that relying solely on observation information yields limited generalization, particularly under high noise, confirming that background priors are critical for robust AI-Var \cite{kalman_1960}.
\item \textbf{Multilayer Perceptron (MLP)}
...
\end{table}
\noindent
At each time step, Gaussian noise $\epsilon \sim \mathcal{N}(0, \sigma^2 I)$ was added with $\sigma \in \{0.05, 0.10, 0.50, 1.00\}$. The $x$ mode provides minimal information, $xy$ offers redundancy and stronger constraints, while $x^2$ introduces nonlinearity that amplifies noise but removes sign information (see Appendix 
\hyperref[sec:app_data]{A.1}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\linewidth]{pipe_eval.png}
...
\end{enumerate}
\noindent
Detailed loss curves for individual configurations are provided in Appendix\hyperref[sec:app_training]{A.2}, including multi-panel grids showing convergence per observation mode and noise level. Section~\ref{sec:4.3} examines the post-assimilation accuracy and stability metrics in greater depth.
...
Effective data assimilation in chaotic systems requires preserving not only instantaneous accuracy but also the \textbf{global geometric structure} of the underlying attractor. While short-window RMSE metrics capture local trajectory fidelity, they do not guarantee that the learned analysis functional reproduces the correct attractor manifold over extended time horizons. This distinction between \textit{local correctness} (accurate state estimates at individual time steps) and \textit{global structural fidelity} (preservation of the attractor's topology and geometry) is critical for chaotic systems, where small deviations can compound into qualitatively different long-term behavior.
\noindent
To rigorously assess global attractor preservation, we employ two complementary diagnostics that address distinct aspects of structural accuracy: \textbf{global-normalized Hausdorff distance}, which quantifies geometric alignment between analysis and truth manifolds, and \textbf{lobe occupancy discrepancy}, which validates that trajectories visit both wings of the butterfly-shaped Lorenz-63 attractor with truth-consistent frequency.

\paragraph{Global-Normalized Hausdorff Distance}

The Hausdorff distance $H(\mathcal{A}_1, \mathcal{A}_2)$ measures the maximum distance from any point in one set to the closest point in another set, providing a symmetric metric of geometric similarity between two point clouds. To enable rigorous cross-regime comparison, we adopt a \textbf{global normalization scheme}: the normalized Hausdorff distance $\tilde{H}_{\text{global}}$ is computed using a \textbf{single, fixed attractor diameter} extracted from long-trajectory truth data, rather than regime-dependent or noise-dependent local diameters. Formally:
\[
\tilde{H}_{\text{global}} = \frac{H(\mathcal{A}_{\text{analysis}}, \mathcal{A}_{\text{truth}})}{\text{diam}(\mathcal{A}_{\text{truth, global}})},
\]
where $\text{diam}(\mathcal{A}_{\text{truth, global}})$ is computed once from a representative long truth trajectory and used consistently across all regimes, architectures, and noise levels. This ensures that $\tilde{H}_{\text{global}}$ reflects \textit{absolute} geometric fidelity on a consistent scale, making comparisons directly interpretable without confounding from trajectory-specific scaling artifacts.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{attractor_metrics_resample_fixedmean.png}
\caption{Global-normalized Hausdorff distances ($\tilde{H}_{\text{global}}$) for Resample and FixedMean regimes at four noise levels ($\sigma \in \{0.05, 0.10, 0.50, 1.00\}$) in the $xy$ observation mode. Each point represents the median $\tilde{H}_{\text{global}}$ across test trajectories, with error bars indicating interquartile range (IQR). Resample consistently achieves low geometric deviation ($\tilde{H}_{\text{global}} \approx 0.32$) with tight dispersion, demonstrating superior structural fidelity independent of noise level. FixedMean exhibits substantially higher deviation ($\tilde{H}_{\text{global}} \approx 1.50$) and increased variance, particularly at $\sigma \geq 0.50$, symptomatic of structural collapse and attractor escape. Baseline regime is omitted from this analysis due to frequent trajectory collapse and inability to maintain chaotic dynamics over the required integration window for attractor reconstruction.}
\label{fig:global_hausdorff}
\end{figure}
\noindent
Figure~\ref{fig:global_hausdorff} demonstrates that Resample maintains consistently low $\tilde{H}_{\text{global}}$ values across all noise levels, approximately 4.7× lower than FixedMean. The tight clustering of Resample values (narrow IQR) reflects robust, noise-invariant geometric adherence. In contrast, FixedMean exhibits both elevated median deviation and increasing variance at higher noise ($\sigma \geq 0.50$), symptomatic of structural collapse and attractor escape. The flat response of Resample to increasing noise confirms that dynamic background updates provide a noise-robust geometric anchor, preventing drift from the true manifold.
\noindent
Baseline regime results are omitted from this figure because the absence of background priors frequently leads to trajectory collapse---the analysis drifts toward fixed points or exhibits non-chaotic behavior that cannot sustain the extended integration required for attractor geometry diagnostics. This failure underscores the fundamental necessity of incorporating dynamical system structure (via background distributions) into the learning process for chaotic DA.

\paragraph{Lobe Occupancy Discrepancy: Global Topology Validation}

While Hausdorff distance quantifies local manifold alignment, it does not guarantee that trajectories explore the attractor's global structure correctly. The Lorenz-63 attractor exhibits a characteristic \textit{double-lobe} ("butterfly") topology, with trajectories alternating between left ($x < 0$) and right ($x > 0$) wings via crossings of the separatrix. A robust assimilation scheme must preserve this global topology: analysis trajectories should visit both lobes with truth-consistent frequency, ensuring correct separatrix crossings.
\noindent
We quantify this via \textbf{lobe occupancy analysis}. For each trajectory, we classify time steps into left lobe ($x < 0$) versus right lobe ($x > 0$) and compute the fractional occupancy of each lobe. The \textbf{lobe occupancy discrepancy} is defined as:
\[
\Delta_{\text{lobe}} = \left| \frac{n_{\text{left}}^{\text{analysis}}}{N} - \frac{n_{\text{left}}^{\text{truth}}}{N} \right|,
\]
where $n_{\text{left}}$ denotes the count of time steps in the left lobe and $N$ is the total trajectory length. A robust assimilation scheme should yield $\Delta_{\text{lobe}} \approx 0$, indicating balanced wing visitation and proper global topology preservation.

\begin{figure} [H]
\centering
\includegraphics[width=\textwidth]{lobe_occupancy_diff_heatmap.png}
\caption{Lobe occupancy discrepancy ($\Delta_{\text{lobe}}$) for all regimes and observation modes across four noise levels ($\sigma \in \{0.05, 0.10, 0.50, 1.00\}$). Color intensity indicates the absolute difference between truth and analysis lobe fractions. Light cells correspond to near-perfect lobe matching ($\Delta_{\text{lobe}} \approx 0$), whereas dark red indicates substantial imbalance. Resample models achieve uniformly low discrepancies across modes, indicating correct separatrix crossings and global attractor coverage. FixedMean shows moderate bias ($\Delta_{\text{lobe}} \approx 0.10$–0.15), particularly at higher noise. Baseline performance varies by mode, with the $x^2$ case exhibiting the highest discrepancies.}

\label{fig:lobe_discrepancy}
\end{figure}
\noindent
Figure~\ref{fig:lobe_discrepancy} reveals stark regime-dependent differences in global structural fidelity. Resample achieves near-zero lobe occupancy discrepancy ($\Delta_{\text{lobe}} < 0.02$) across all observation modes and noise levels, confirming that dynamic background updates enable correct separatrix crossing and balanced wing visitation. The $xy$ mode under Resample is particularly robust, maintaining $\Delta_{\text{lobe}} < 0.02$ even at $\sigma=1.00$, indicating that dual-component observations provide sufficient constraint for global topology preservation.
\noindent
In contrast, FixedMean exhibits substantial lobe imbalances, especially at higher noise levels and for single-component observations ($x$ and $x^2$ modes). For instance, at $\sigma=0.50$ under the $x$ mode, FixedMean shows $\Delta_{\text{lobe}} \approx 0.15$, indicating that approximately 15\% of the trajectory incorrectly accumulates in one lobe. This aligns with observed attractor escape failures: without dynamic updates, the static prior biases the analysis toward one wing, preventing proper chaotic evolution across the separatrix.
\noindent
The Baseline regime demonstrates intermediate behavior: while not as catastrophic as FixedMean, it still exhibits mode-dependent lobe bias. The $x^2$ mode under Baseline shows elevated $\Delta_{\text{lobe}}$ at all noise levels, likely because the nonlinear observation function removes sign information, obscuring the lobe transition signal and hindering global trajectory reconstruction. This mode-dependent sensitivity highlights the interaction between observation information content and the ability to infer global attractor structure without explicit background guidance.

\paragraph{Synthesis: Dual Metrics for Structural Accuracy}

Together, global-normalized Hausdorff distance and lobe occupancy discrepancy provide a complete characterization of structural accuracy in chaotic attractor reconstruction:

\begin{enumerate}
\item \textbf{Hausdorff ($\tilde{H}_{\text{global}}$):} Quantifies local manifold alignment---how closely the analysis trajectory adheres to the truth manifold at each point. Low $\tilde{H}_{\text{global}}$ ensures geometric fidelity but does not guarantee global coverage.
\item \textbf{Lobe Occupancy ($\Delta_{\text{lobe}}$):} Quantifies global topology preservation---whether the analysis visits both wings of the attractor with the correct frequency. Low $\Delta_{\text{lobe}}$ ensures separatrix crossing fidelity and balanced exploration.
\end{enumerate}
\noindent
Resample achieves excellence on both metrics: low $\tilde{H}_{\text{global}}$ (local geometry) and low $\Delta_{\text{lobe}}$ (global coverage). This dual success demonstrates that dynamic background updates enable the learned functional to \textit{simultaneously} track the local chaotic manifold \textit{and} reproduce the global double-wing structure. In contrast, FixedMean fails on both fronts, exhibiting elevated Hausdorff distances and substantial lobe imbalances. Baseline shows partial success but lacks the precision and robustness needed for operational deployment.
\noindent
These findings confirm that preserving attractor geometry in chaotic DA requires \textit{both} accurate local corrections (captured by RMSE and Hausdorff) \textit{and} global structural awareness (captured by lobe occupancy). The Resample regime satisfies both requirements across all tested conditions, validating stochastic background resampling as a fundamental requirement for faithful attractor reconstruction. Additional visual diagnostics, including phase-space projections and multi-regime attractor grids, are provided in Appendix 
\hyperref[sec:app_attractor]{A.3}
 for detailed qualitative assessment.

\subsubsection{Component‑Wise Correction Patterns}

Define correction $c(t)=x^{a}(t)-x^{b}(t)$ (analysis minus background) and analysis error $e(t)=x^{a}(t)-x^{\text{true}}(t)$. Residual plots (Appendix Fig.~\ref{fig:app_componentwise}) show:
\begin{itemize}
\item \textbf{$Z$ component:} Smooth, low‑frequency corrections consistent with slower decay in $dz/dt=xy-\beta z$.
\item \textbf{$X,Y$ components:} Higher‑variance, rapid adjustments tied to chaotic turnover; GRU/LSTM reduce correction variance ($\sigma_c\approx0.1$–0.2) relative to MLP, indicating temporal gating mitigates spurious high‑frequency updates.
\end{itemize}

\subsubsection{Summary}

\begin{enumerate}
\item Resample stabilizes geometric deviation (flat $\tilde{H}$ vs $\sigma$) while FixedMean amplifies distortion.
\item Baseline provides a middle ground but lacks the precision needed for robust chaotic tracking.
\item Geometric fidelity (low $\tilde{H}$) correlates with lower RMSE and sustained manifold adherence, supporting resampling as a structural, not incidental, requirement.
\end{enumerate}
\noindent
...
\end{itemize}
\noindent
Using sequence length $L = 10$--15 time steps for recurrent models. Longer sequences yield diminishing returns and increased computational cost without improving accuracy.
...
\end{enumerate}
\noindent
 Validate $B$ accuracy to within $\pm 10\%$ of the true covariance scale. For operational systems, use ensemble-derived covariance estimates and update them regularly. The Resample regime's robustness to $B$ misestimation makes it preferable when covariance estimates are uncertain.
...
\end{itemize}
\noindent
Maintain observation coverage above 75\% for reliable performance. If higher sparsity is unavoidable, use ensemble averaging or data augmentation techniques.
...
\vspace{-0.3cm}
This study aims to contribute to the investigation of neural network architectures for data assimilation in the Lorenz–63 system, focusing on how temporal modeling and ensemble-based strategies support the reconstruction of chaotic trajectories. Experiments with feed-forward and recurrent architectures under fixed and resampled background conditions showed that models capturing temporal dependencies, such as Gated Recurrent Units (GRU) and Long Short-Term Memory (LSTM) networks, consistently outperformed the simpler Multi-Layer Perceptron (MLP) in terms of performance when utilizing AI-Var in dynamical systems. The GRU exhibited particularly stable and generalizable learning, indicating that its gating mechanisms effectively reduce overfitting and improve adaptability with limited data. Resampling the background mean during training enhanced reconstruction quality by better representing evolving background uncertainty and the system’s stochasticity, whereas the fixed-mean formulation often produced divergent or flattened trajectories. While a hypothetical training-time penalty toward the true state (a ``truth-anchor'') could improve stability in contrived diagnostics, such an approach is not realistic for operational data assimilation as the true state is unavailable. No truth-anchor was used in the experiments reported here; all training minimized the self-supervised 3D-Var objective without access to $x_{\mathrm{true}}$.
...
This appendix provides comprehensive visual diagnostics for attractor geometry preservation, complementing the quantitative metrics presented in Section~\ref{sec:4.5}. The figures below include detailed phase-space projections, architecture-specific comparisons, and multi-regime attractor grids.
