...
$$
where $b$ is the prior background state, $y$ is the observation vector, $H$ is the observation operator that maps the state to the observed space, and $B$ and $R$ represent the prior and observation error covariances, respectively \cite{ai_da_fablet}. The solution requires computationally expensive iterative solvers or matrix inversions during inference, which poses a limitation for real-time or embedded applications.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{attractor_metrics_all_regimes.png}
...
\includegraphics[width=\textwidth]{ATTRACTOR_GRID_Baseline_x.png}
\caption{Baseline, $x$ mode}
\label{fig:app_attractor_baseline_x}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.32\textwidth}
\includegraphics[width=\textwidth]{ATTRACTOR_GRID_Baseline_xy.png}
\caption{Baseline, $xy$ mode}
\label{fig:app_attractor_baseline_xy}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.32\textwidth}
\includegraphics[width=\textwidth]{ATTRACTOR_GRID_Baseline_x2.png}
\caption{Baseline, $x^2$ mode}
\label{fig:app_attractor_baseline_x2}
\end{subfigure}
\caption{(Appendix) Attractor geometry for Baseline regime across three observation modes at multiple noise levels ($\sigma \in \{0.05, 0.10, 0.50, 1.00\}$). Each grid shows XY (left) and YZ (right) projections with truth (gray) and analysis (blue) trajectories overlaid. Baseline demonstrates partial geometric adherence but exhibits visible lobe imbalances and trajectory drift at higher noise, particularly for the $x^2$ mode where nonlinear observations obscure separatrix crossing cues.}
...
\centering
\begin{subfigure}[b]{0.32\textwidth}
\includegraphics[width=\textwidth]{ATTRACTOR_GRID_FixedMean_x.png}
\caption{FixedMean, $x$ mode}
\label{fig:app_attractor_fixedmean_x}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.32\textwidth}
\includegraphics[width=\textwidth]{ATTRACTOR_GRID_FixedMean_xy.png}
\caption{FixedMean, $xy$ mode}
\label{fig:app_attractor_fixedmean_xy}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.32\textwidth}
\includegraphics[width=\textwidth]{ATTRACTOR_GRID_FixedMean_x2.png}
\caption{FixedMean, $x^2$ mode}
\label{fig:app_attractor_fixedmean_x2}
\end{subfigure}
\caption{(Appendix) Attractor geometry for FixedMean regime across three observation modes. FixedMean exhibits severe geometric distortion at moderate to high noise levels, with analysis trajectories (blue) deviating substantially from truth (gray). Visible attractor escape is evident in several panels, where the analysis drifts toward fixed points or exhibits fragmented, non-chaotic behavior. Lobe occupancy discrepancies manifest as visibly unbalanced wing visitation, confirming the quantitative findings in Figure~\ref{fig:lobe_discrepancy}.}
...
\centering
\begin{subfigure}[b]{0.32\textwidth}
\includegraphics[width=\textwidth]{ATTRACTOR_GRID_Resample_x.png}
\caption{Resample, $x$ mode}
\label{fig:app_attractor_resample_x}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.32\textwidth}
\includegraphics[width=\textwidth]{ATTRACTOR_GRID_Resample_xy.png}
\caption{Resample, $xy$ mode}
\label{fig:app_attractor_resample_xy}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.32\textwidth}
\includegraphics[width=\textwidth]{ATTRACTOR_GRID_Resample_x2.png}
\caption{Resample, $x^2$ mode}
\label{fig:app_attractor_resample_x2}
\end{subfigure}
\caption{(Appendix) Attractor geometry for Resample regime across three observation modes. Resample demonstrates exceptional geometric fidelity across all modes and noise levels, with analysis trajectories (blue) closely tracking truth (gray) and preserving the double-lobe structure. Balanced wing visitation is visually evident, confirming near-zero lobe occupancy discrepancies. Even at $\sigma=1.00$ and under the challenging $x^2$ mode, Resample maintains chaotic coherence and proper separatrix crossings, validating dynamic background updates as essential for global topology preservation.}
\label{fig:app_attractor_grid_resample}
\end{figure}

The composite grids provide compelling visual evidence for the quantitative metrics presented in Section~\ref{sec:4.5}. Resample grids show tight overlay between truth and analysis across all conditions, with both lobes equally populated and smooth transitions between wings. FixedMean grids reveal fragmented, distorted attractors with visible lobe bias and occasional escape to non-chaotic regimes. Baseline grids occupy an intermediate position, showing reasonable local alignment but with noticeable drift and incomplete double-lobe coverage at higher noise.
