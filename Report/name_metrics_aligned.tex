...
\caption{Heatmap of lobe occupancy discrepancy ($\Delta_{\text{lobe}}$) across regimes (rows: Baseline, FixedMean, Resample) and observation modes (columns: $x$, $xy$, $x^2$) at four noise levels ($\sigma \in \{0.05, 0.10, 0.50, 1.00\}$). Color intensity indicates absolute discrepancy magnitude; lighter colors (white/pale pink) represent near-perfect occupancy matching ($\Delta_{\text{lobe}} \approx 0$), while darker colors (dark red/brown) indicate significant lobe imbalance. Resample (bottom row) consistently achieves the lowest $\Delta_{\text{lobe}}$ across all modes and noise levels (light colors), demonstrating global shape preservation via correct separatrix crossings. FixedMean (middle row) shows elevated discrepancies ($\Delta_{\text{lobe}} \approx 0.10$--0.15), particularly for $x$ and $x^2$ modes at high noise, symptomatic of attractor escape and lobe bias. Baseline (top row) exhibits intermediate performance with strong mode dependence, with the $x^2$ mode showing particularly high discrepancies (dark red, $\Delta_{\text{lobe}} \approx 0.44$).}
