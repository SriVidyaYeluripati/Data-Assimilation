...
All data are derived from the canonical Lorenz-63 chaotic system \cite{lorenz_1963}, integrated using a fourth-order Runge--Kutta scheme with time step $\Delta t = 0.01$ and standard parameters ($\sigma = 10$, $\rho = 28$, $\beta = 8/3$). A total of 1500 trajectories were generated, each containing 200 time steps. The dataset was split into training (1000 trajectories), validation (250 trajectories), and test (250 trajectories) sets to ensure robust evaluation.

\begin{figure}[H]
\centering
\includegraphics[width=0.75\linewidth]{Dataset.png}
\caption{Dataset distribution across train, validation, and test splits for all observation modes and noise levels. Each configuration contributes approximately $2.94 \times 10^5$ effective samples, ensuring balanced evaluation.}
...
\begin{table}[H]
\centering
\caption{Practical recommendations for AI-Variational Data Assimilation in chaotic systems.}
