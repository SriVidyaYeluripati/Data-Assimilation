...
- "unclear observation mode (x / xy / x²)" - Added clear labels h(x) = x₁, h(x) = (x₁,x₂), h(x) = x₁²
- Missing noise level labels: Added σ values

This script regenerates the ORIGINAL figure with corrections.
Original: Report/4_4a_post_assimilation_rmse.png
Output: Report/figures_new/4_4a_post_assimilation_rmse_logscale.png
"""
...
def generate_figure():
    """Generate log-scale boxplot for RMSE by observation mode."""
    df = load_resample_data()
    if df is None:
        print("Error: Could not load resample data")
        return
    
    # Create figure with 3 subplots (one per observation mode)
    fig, axes = plt.subplots(1, 3, figsize=(14, 5))
    fig.suptitle('Post-Assimilation RMSE by Observation Mode (Resample Regime)\n'
                 'All Architectures Combined — Log Scale', fontsize=12)
    
    # Hans comment: clarify observation modes with proper notation
    modes = ['x', 'xy', 'x2']
    mode_labels = ['$h(x) = x_1$\n(Partial Linear)', 
                   '$h(x) = (x_1, x_2)$\n(Bilinear Coupled)', 
                   '$h(x) = x_1^2$\n(Nonlinear)']
    ...
    for idx, (mode, label, color) in enumerate(zip(modes, mode_labels, colors)):
        ax = axes[idx]
        
        # Filter data for this observation mode
        mode_data = df[df['mode'] == mode]
        
        # Collect RMSE values for each noise level (aggregated across architectures)
        box_data = []
        positions = []
        for i, sigma in enumerate(noise_levels):
            sigma_data = mode_data[mode_data['sigma'] == sigma]['rmse_a'].values
            if len(sigma_data) > 0:
