...
The covariance matrices $B$ and $R$ encode critical information about the relative reliability of background and observations. To provide available knowledge about the underlying system, these matrices must be specified or estimated: $B$ is typically derived from long-term ensemble statistics from the attractor, while $R$ is determined by instrument characteristics. In this study, $B$ is computed empirically from an ensemble of Lorenz-63 trajectories, and $R = \sigma_{\text{obs}}^2 I$ is diagonal with variance determined by the noise level.
...
This formulation is \emph{self-supervised}: no ground truth state $x^{\text{true}}$ or pre-computed analysis $x^a$ is required during training. The network learns to balance the two terms of the cost function through gradient descent, effectively learning the structure of the variational problem from data. The true state is used only for offline evaluation metrics such as RMSE, enabling systematic assessment of reconstruction accuracy without introducing supervised bias during training.
...
Background priors are generated from an independent long-term ensemble. Concretely, an ensemble of size $E = 10000$ is constructed by sampling states from a long Lorenz-63 trajectory at intervals exceeding the decorrelation time. The background ensemble mean $\bar{x}_{\text{ens}}$ and covariance $B_{\text{ens}}$ are computed from this ensemble.

Two background conditioning regimes are investigated:

\paragraph{FixedMean Regime.} The background mean $\bar{x}$ and covariance $B$ are set to the static ensemble values $(\bar{x}_{\text{ens}}, B_{\text{ens}})$ and remain constant throughout training. This configuration stresses the network by forcing it to correct from a fixed, potentially biased prior.
...
\caption{Mean convergence envelopes across training regimes. The Resample regime (blue) achieves the fastest and most stable convergence, reducing loss to approximately 1.5\% of the initial value by epoch 30. FixedMean (orange) shows rapid initial descent, stabilizing at approximately 25\% of initial loss, but exhibits instability at high noise levels during evaluation. Baseline (green) converges slowly, retaining approximately 89\% of initial error after 30 epochs. Shaded regions indicate standard deviation across observation modes and noise levels.}
...
\caption{Post-assimilation RMSE distributions (logarithmic scale) for MLP, GRU, and LSTM across observation modes in the Resample regime. Grouped bar charts show mean RMSE for each architecture, with value labels for precise comparison. The ordering $\text{RMSE}(h(x)=x_1) < \text{RMSE}(h(x)=(x_1,x_2)) < \text{RMSE}(h(x)=x_1^2)$ reflects learning difficulty rather than information content: the linear observation $h(x)=x_1$ is easiest for the network to learn, while the nonlinear $h(x)=x_1^2$ mode is most challenging due to sign ambiguity. Differences between architectures are modest ($<$15\%), indicating inconclusive evidence for architectural superiority.}
...
\noindent
The structure of the observation operator substantially affects reconstruction accuracy. Our analysis reveals a hierarchy that reflects \emph{learning difficulty} rather than information content:
\[
\text{RMSE}(x) < \text{RMSE}(xy) < \text{RMSE}(x^2).
\]
This ordering may seem counterintuitive---the $xy$ mode provides more information about the state than $x$---but the network finds the simpler linear observation $h(x)=x_1$ easier to learn. The nonlinear $h(x)=x_1^2$ mode is most challenging due to sign ambiguity, where both $+|x_1|$ and $-|x_1|$ produce identical observations.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures_new/4_4a_post_assimilation_rmse_logscale.png}
\caption{Post-assimilation RMSE (logarithmic scale) by observation mode and noise level $\sigma_{\text{obs}}$. Four panels show results for each noise level, with bars grouped by observation operator $h(x)$ and colored by architecture. The hierarchy $\text{RMSE}(x) < \text{RMSE}(xy) < \text{RMSE}(x^2)$ emerges, where $x$ denotes $h(x)=x_1$, $xy$ denotes $h(x)=(x_1,x_2)$, and $x^2$ denotes $h(x)=x_1^2$. This ordering reflects the network's learning difficulty rather than information content of the observations.}
...
\caption{Temporal evolution of Euclidean error over the assimilation window for FixedMean and Resample regimes. Resample (shown in detail) demonstrates stable error evolution across test trajectories. FixedMean exhibits catastrophic divergence in 94\% of configurations, rendering temporal analysis unreliable for that regime.}
\label{fig:error_evolution}
\end{figure}

\noindent
The Resample regime achieves rapid error reduction within approximately 50 time steps (roughly half a Lyapunov time), stabilizing at a low steady-state level. The error trajectory remains smooth and bounded, indicating that the learned correction functional successfully tracks the chaotic manifold.

\paragraph{Trajectory Reconstruction Quality.} Figure~\ref{fig:trajectory_fidelity} illustrates the analysis quality for a representative test trajectory.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{figures_new/4_5a_trajectory_fidelity_corrected.png}
\caption{Trajectory reconstruction comparison for a test sample. The neural network $f_\theta$ produces an analysis estimate (orange) that closely tracks the true state (blue), with the background forecast (green) shown for reference. The analysis functional maps from observations and background to a refined state estimate.}
...
\begin{figure}[H]
\centering
\includegraphics[width=1.0\textwidth]{figures_new/4_6a_background_sampling_stability_corrected.png}
\caption{Stability comparison between Resample and FixedMean regimes. The Resample regime achieves 100\% success rate across all 36 configurations (3 architectures $\times$ 3 modes $\times$ 4 noise levels), whereas FixedMean exhibits a 94\% failure rate (only 2/36 configurations succeed). This stark difference demonstrates that stochastic background resampling is essential for stable learning in this setting.}
\label{fig:background_stability}
\end{figure}

The most critical finding of this study concerns regime stability. The FixedMean approach, which uses a static background mean throughout training, fails catastrophically in 94\% of configurations. Only 2 out of 36 FixedMean configurations produce bounded trajectories; the remainder diverge with RMSE values exceeding 100. In contrast, the Resample regime achieves 100\% success, producing stable, bounded analysis estimates across all configurations. This finding strongly motivates the use of stochastic background updates for neural network-based data assimilation.
...
\noindent
A critical finding concerns the \emph{improvement} achieved by the learned analysis over the background forecast. Our analysis reveals that the mean improvement is approximately zero:
\[
\text{Improvement} = \frac{\text{RMSE}_{\text{background}} - \text{RMSE}_{\text{analysis}}}{\text{RMSE}_{\text{background}}} \approx 0\%
\]
across all observation modes in the Resample regime. In some configurations, the analysis RMSE is slightly \emph{higher} than the background RMSE, indicating that the network provides no meaningful correction beyond the prior forecast.

\noindent
This result is neither a success nor a failure---it is genuinely inconclusive. The learned functional $f_\theta$ produces bounded, stable analysis estimates that remain on the attractor, but these estimates are not systematically better than the background. This may indicate that the AI-Var approach requires further refinement for this problem setting, or that the Lorenz-63 system's simple structure provides insufficient benefit from learned corrections. As Hans emphasized in our discussions, inconclusive results are acceptable in a pilot study; they indicate that more investigation is needed rather than that the approach has failed.
...
\caption{(Appendix) Training and validation loss convergence grid for the Resample regime. Rows correspond to observation modes ($h(x)=x_1$, $h(x)=(x_1,x_2)$, $h(x)=x_1^2$); columns correspond to noise levels ($\sigma_{\text{obs}} \in \{0.05, 0.1, 0.5, 1.0\}$). Solid lines show training loss; dashed lines show validation loss. All architectures (MLP, GRU, LSTM) converge smoothly across all configurations. This figure is a supporting diagnostic and does not change the main conclusions based on RMSE.}
