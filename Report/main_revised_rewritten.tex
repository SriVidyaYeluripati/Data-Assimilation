...
\noindent
When distributions exhibit heavy tails due to catastrophic failures such as attractor escape, we additionally report the \emph{Root Median Squared Error} (RMdSE):
\begin{equation}
\text{RMdSE} = \sqrt{\text{median}_{i} \|\hat{x}^a_i - x^{\text{true}}_i\|^2},
\label{eq:rmdse}
\end{equation}
which provides a robust alternative to RMSE that is less sensitive to outliers. RMdSE characterizes typical performance even when a small fraction of runs diverge catastrophically. Boxplot visualizations employ logarithmic y-axes to better reveal the distribution structure when outliers span multiple orders of magnitude.
