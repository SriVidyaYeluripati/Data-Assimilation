...
\caption{(Appendix) Training and validation loss convergence grid for the Resample regime. Rows correspond to observation modes ($h(x)=x_1$, $h(x)=(x_1,x_2)$, $h(x)=x_1^2$); columns correspond to noise levels ($\sigma_{\text{obs}} \in \{0.05, 0.1, 0.5, 1.0\}$). Solid lines show training loss; dashed lines show validation loss. All architectures (MLP, GRU, LSTM) converge smoothly across all configurations.}
